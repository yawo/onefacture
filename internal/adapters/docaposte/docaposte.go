// Package docaposte is the Docaposte (SERES) PDP adapter.
package docaposte
...
func New() *Adapter {
	return &Adapter{client: sandbox.Client{
		Name:       "docaposte",
		BaseURL:    os.Getenv("ONEFACTURE_DOCAPOSTE_BASE_URL"),
		SubmitPath: envOr("ONEFACTURE_DOCAPOSTE_SUBMIT_PATH", "/invoices"),
		StatusPath: envOr("ONEFACTURE_DOCAPOSTE_STATUS_PATH", "/invoices/{pa_ref}/status"),
		WebhookKey: os.Getenv("ONEFACTURE_DOCAPOSTE_WEBHOOK_KEY"),
		Auth:       sandbox.Auth{Scheme: "Bearer", Token: os.Getenv("ONEFACTURE_DOCAPOSTE_API_TOKEN")},
	}}
}
